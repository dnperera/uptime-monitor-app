...
workers.rotateLogs = () => {
  ...
  _logs.list(false, (err, listOfLogs) => {
    if (!err && listOfLogs.length > 0) {
      listOfLogs.forEach(logName => {
        //compress the data to a different file
        var logId = logName.replace(".log", "");
        var newFileId = logId + "-" + Date.now();
        _logs.compress(logId, newFileId, err => {
          if (!err) {
            //Trunkating the original log files once the content taken out
            ...
            console.log("Error in compressing one of the log file ", err);
          }
        });
      });
    } else {
...
//Timer to execute log compression process everyday
...
//write previous checkdata,checkoutcome ,state to a log file

workers.log = (checkData, checkOutcome, state, alertWarranted, timeofCheck) => {
  //Form the log data
  var logData = { checkData, checkOutcome, state, alertWarranted, timeofCheck };
  //Convert the logData to JSON string
  var logString = JSON.stringify(logData);
  //append the log string to the file
  //checkData.id -log file name
  _logs.append(checkData.id, logString, err => {
    if (!err) {
      console.log("Check data write to the log file successfully!!");
    } else {
      console.log("Error :Saving check data to the file failed !!");
    }
  });
};
//initialize worker
workers.init = () => {
  ...
  //Call the loop so checks will execute later on regularly
  workers.loop();

  //Compress all the logs immediately server starts
  workers.rotateLogs();

  //call the log compression loop do logs will be compress as scheduled
  workers.logRotationLoop();
};
